...
def test_accepted_layouts():
    ...
    assert to_grid(np.zeros((4, 1, 8, 8))).shape == (16, 16), "(B, 1, H, W) -> 2x2"
...
def test_the_ambiguous_shapes_raise_and_say_how_to_fix_it():
    ...
    assert to_grid(np.zeros((3, 8, 8)), channels="none").shape == (16, 16), "3 grayscale -> 2x2, one blank"
...
def test_tiling_puts_images_in_row_major_order():
    ...
    g = to_grid(imgs, rows=2, cols=2, channels="none", vmin=0, vmax=3)
...
def test_scale_each():
    ...
    together = to_grid(imgs, rows=1, cols=2, channels="none")
    apart = to_grid(imgs, rows=1, cols=2, channels="none", scale_each=True)
    assert together.tolist() == [[0, 26, 0, 255]], "one global range"
    assert apart.tolist() == [[0, 255, 0, 255]], "each image on its own range"
    assert to_grid(imgs, rows=1, cols=2, channels="none", scale_each=True, vmin=0, vmax=1).tolist() \
...
def test_pad_value():
    ...
    assert (to_grid(imgs, grid_size=(1, 2), channels="none", pad_value=255)[:, 4:] == 255).all()
